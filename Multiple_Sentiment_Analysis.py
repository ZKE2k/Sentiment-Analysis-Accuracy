...
names, instances, labels = load_data_from_csv(traind)
testnames, testinstances, testlabels = load_data_from_csv(testd)
...
# superior accuracy, as well as this, it can be used to determine what type of dataset the csv is.
...
if question == "Yes" or == "yes":
    ...
    df_train_data = pd.read_csv(rawtrain, sep='\t', header=None)
    # This loads the raw test data for the algorithms as a dataframe.
    df_test_data = pd.read_csv(rawtest, sep='\t', header=None)
    # Loads the sentiment lexicon as a pandas dataframe.
    df_sentiment_lexicon = pd.read_csv(lexicon, delimiter='\t', header=None)
